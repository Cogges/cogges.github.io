 ...
 *
 */
function listElementEvents(feed, divId, listTitle, startswith, limit) {
    var events = document.getElementById(divId);
...
function makeApiCall(render, calendar_id, list_id, listTitle, results_window, limit, startswith) {
    gapi.client.load('calendar', 'v3', function() { // load the calendar api (version 3)
        var request = gapi.client.calendar.events.list({
            'calendarId': calendar_id,
            'maxResults': results_window, // show max of 20 events
        ...
        request.execute(function(resp) {
            render(resp, list_id, listTitle, startswith, limit);
        });
    });
}
